...
C++17 builds large systems from value types, RAII, templates, overloads, exceptions, and carefully hidden resource edges. Musi keeps the taste for named operations and static promises, but separates stored data from behaviors: records and data describe things, classes describe operations, effects and foreign named parts mark edge work.
...
- Drop inheritance-first and overload-first reflexes when a record, data variant, or clear instance says the same thing plainly.
- Keep examples tied to normal work: receipts, routes, files, animals, people, rooms, and services.

## First common mistake

Musi `class` is closer to a concept or trait than a C++ class. Records and data store data. Instances give behavior. If you need a person, car, invoice, or package, start with records or data. If you need any type that can compare, show, encode, or step, use a class with instances.
