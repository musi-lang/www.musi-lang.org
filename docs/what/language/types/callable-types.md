...
description: "Read pure and effectful function types as part of normal type syntax."
group: "Types"
section: "Types"
order: 18
slug: "callable-types"
...
A shop may pass a discount rule into checkout. A shelter may pass an adoption filter into a search page. The calling code does not need to know which rule was chosen, only that it has the right callable form.

## Keep signatures readable

If a callable type becomes long, name it. A named type such as `PriceRule` or `AnimalFilter` can make a named part readable again and give the behavior a place in the app language.
