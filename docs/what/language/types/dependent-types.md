...
description: "Use values in type positions when form, size, or protocol state should be visible in the type."
group: "Types"
section: "Types"
order: 23
...
---
Dependent types let types mention values when the form of the program needs that precision. They are useful for sizes, labels, indexes, or other facts that must travel with a type.
...
Do not move every normal value into the type system. A person's age changes, a cart total changes, and a search query is normal data. Dependent information should describe structure, not everyday noise.
